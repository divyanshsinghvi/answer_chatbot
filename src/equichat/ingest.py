...
def persist_facts(store: Store, facts: List["FactRow"], doc_id: str) -> int:
    """Idempotently persist facts for the given doc_id into DuckDB."""
...
def ingest_pdf_with_openai(
    ...
    persist: bool = True,  # <--- NEW: control DB writes
) -> Dict[str, Any]:
    """
    Parse a financial PDF using the OpenAI Responses + Files API and (optionally) persist normalized 'facts' to DuckDB.
    ...
    Returns:
      {
        ...
        "facts_inserted": 42,              # when persist=True
        "facts": [ ... normalized rows ],  # when persist=False (omit insert)
      }
    """
    ...
    # 3) Either return facts only (for parallel workers) OR persist (single writer)
    if not persist:
        # return raw normalized facts so caller can persist in main thread
        payload["facts"] = [
            {
                "doc_id": f.doc_id,
                "company": f.company,
                "metric_key": f.metric_key,
                "metric_variant": f.metric_variant,
                "value": f.value,
                "unit": f.unit,
    ...
    # Persist here (single-threaded contexts like /ingest API)
    with Store(CONFIG.db_path, read_only=False) as store:
        n = persist_facts(store, facts, doc_id)

    payload["facts_inserted"] = n
    return payload
