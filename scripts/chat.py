...
def main() -> int:
    console.print(Panel.fit(f"EquiChat CLI — talks to FastAPI at {API_URL}", style="bold cyan"))
    ...
    while True:
        ...
        if cmd in {"/q", "/quit", "/exit"}:
            break
        if cmd == "/help":
            console.print("[cyan]/reset[/cyan] clear conversation")
        ...
        try:
            resp = requests.post(f"{API_URL}/query", json={"messages": history}, timeout=90)
        ...
        # 3) read normalized schema (tool/sql/rows/answer)
        answer = payload.get("answer") or ""
        tool = payload.get("tool") or "n/a"
        sql = payload.get("sql")
        rows = payload.get("rows") or []

        # render
        console.print(Panel.fit(f"Tool: {tool}", style="bold magenta"))
        if sql:
            console.print(Panel.fit(sql, title="SQL", style="dim"))
        if answer:
            console.print(Panel.fit(answer, title="Answer", style="bold green"))
        else:
            console.print("[yellow]No answer text.[/yellow]")
